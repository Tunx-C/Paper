...
__all__ = [
    "build_iqa_model", "load_state_dict", "make_directory", "save_checkpoint",
    "Summary", "AverageMeter", "ProgressMeter"
]


def build_iqa_model(upscale_factor: int, only_test_y_channel: bool, device: torch.device) -> tuple[PSNR, SSIM]:
    psnr_model = PSNR(upscale_factor, only_test_y_channel)
    ssim_model = SSIM(upscale_factor, only_test_y_channel)
    psnr_model = psnr_model.to(device=device)
    ssim_model = ssim_model.to(device=device)
